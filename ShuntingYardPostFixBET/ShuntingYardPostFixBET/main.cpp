...
/*
* Program Name: ShuntingYardPostFixBET
* Date:         2025-12-8
* Author:       Asher Isgitt
* Module Purpose
* This program takes infix expressions from an input file and:
* uses the Shunting-Yard algorithm to convert them into postfix notation
* builds a Binary Expression Tree (BET) from the postfix expression
* displays the expressions with three different traversal methods
*/
...
TreeNodeClass* ConstructBET(string postFixStr) {
    ...
    for(char token : postFixStr)
    {
        ...
        else
        {
            ...
            parseStack.push(BuildNewNodeObjPtrMethod(token));
        }
    }
...
bool areParensRequired(TreeNodeClass* child, char parentOperator, bool isRightChild) {
    ...
    if (!operatorMapObj.isOperator(parentOperator) || !operatorMapObj.isOperator(child->value)) {
        return false;
    }

    bool childGEParent = operatorMapObj.isStackOperatorHigherPrecedence(parentOperator, child->value);
    if (!childGEParent) {
        // if child precedence is less than parent precedence, parentheses required
        return true;
    }

    bool parentGEChild = operatorMapObj.isStackOperatorHigherPrecedence(child->value, parentOperator);
    if (childGEParent && parentGEChild) {
        // equal precedence means right child of '-' or '/' needs parentheses
        if (isRightChild && (parentOperator == '-' || parentOperator == '/')) return true;
        return false;
    }

    // child precedence greater than parent precedence, no parentheses
    return false;
}
...
void inorder(TreeNodeClass* treeNode) {
    ...
    if (treeNode != nullptr) {
        ...
        // go left
        if (areParensRequired(treeNode->left, treeNode->value, false        )) {
            buildString += '(';
        ...
        // go right
        if (areParensRequired(treeNode->right, treeNode->value, true)) {
            buildString += '(';
